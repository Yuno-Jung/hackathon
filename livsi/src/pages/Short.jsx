...
export default function Short() {
  ...
  const handleSwipeDown = () => {
    ...
    const leastWatchedVideos = ALL_VIDEOS.filter(video => (watchCounts.get(video.id) || 0) === minWatchCount);
    const nextVideo = leastWatchedVideos[Math.floor(Math.random() * leastWatchedVideos.length)];

    setPlaylist(prev => [...prev, nextVideo.id]);
    setWatchCounts(prev => new Map(prev).set(nextVideo.id, (prev.get(nextVideo.id) || 0) + 1));
    ...
    <div>
      ...
      <div className="player-container">
        <div ref={containerRef} className="shorts-player" {...eventHandlers}>
          <div className="video-track" style={{ transform: `translateY(-${currentIndex * 100}%)` }}>
            {videosToRender.map(({ id, position }) => (
              <div key={`${id}-${position}`} className="video-slide" style={{ top: `${position * 100}%`, backgroundColor: getColorById(id) }}>
